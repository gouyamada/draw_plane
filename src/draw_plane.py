...
def interactive_plane_visualizer():
    ...
    # スライダーの定義
    s_x0 = Slider(ax_x0, "x₀", -3.0, 3.0, valinit=init_x0, valstep=0.1)
    s_y0 = Slider(ax_y0, "y₀", -3.0, 3.0, valinit=init_y0, valstep=0.1)
    s_z0 = Slider(ax_z0, "z₀", -3.0, 3.0, valinit=init_z0, valstep=0.1)
    s_A  = Slider(ax_A,  "A",  -5.0, 5.0, valinit=init_A,  valstep=0.1)
    s_B  = Slider(ax_B,  "B",  -5.0, 5.0, valinit=init_B,  valstep=0.1)
    s_C  = Slider(ax_C,  "C",  -5.0, 5.0, valinit=init_C,  valstep=0.1)

    def update(_):
        """
        スライダーの値が変更されたときに呼び出されるコールバック関数
        この関数は、スライダーの値を取得し、平面の方程式を更新して平面を再描画する。
        """
    ...
    def safe_remove(item):
        """
        指定されたアイテムを削除するヘルパー関数
        """
        if item is not None:
            try:
                item.remove()
